...
# ✅ Use currently active models (update if needed)
...
def fix_code_errors(code):
    messages = [{"role": "system", "content": FIX_ERRORS_PROMPT},
...
def improve_code_quality(code):
    messages = [{"role": "system", "content": IMPROVE_CODE_PROMPT},
...
def generate_code_from_prompt(prompt):
    messages = [{"role": "system", "content": GENERATE_CODE_PROMPT},
                {"role": "user", "content": prompt}]
    result = call_groq(MODEL_CODE_GEN, messages)
    return result

def build_ui_from_description(desc):
    messages = [{"role": "system", "content": UI_BUILDER_PROMPT},
...
def explain_code_snippet(code):
    messages = [{"role": "system", "content": EXPLAIN_CODE_PROMPT},
                {"role": "user", "content": f"Explain this code:\n{code}"}]
    result = call_groq(MODEL_EXPLAIN_IDEA, messages)
    return result

def generate_project_idea(topic=None):
    user_msg = f"Generate a unique project idea about: {topic}" if topic else "Generate a random creative project idea."
    messages = [{"role": "system", "content": CREATE_IDEA_PROMPT},
                {"role": "user", "content": user_msg}]
    result = call_groq(MODEL_EXPLAIN_IDEA, messages)
    return result

def convert_code(source_code, source_lang, target_lang):
    prompt = CONVERT_CODE_PROMPT.format(source_lang=source_lang, target_lang=target_lang, code=source_code)
    messages = [{"role": "system", "content": "You are a code conversion expert."},
                {"role": "user", "content": prompt}]
    result = call_groq(MODEL_CODE_GEN, messages)
    return result if not (result and result.startswith("ERROR")) else "Conversion failed."